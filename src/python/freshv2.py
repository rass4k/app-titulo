...
def main():
    ...
    with mp_face_mesh.FaceMesh(
        ...
        while True:
            ...
            cv2.putText(frame_final, f'Pestaneos: {contador_pestaneos}', (30, 50),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)

            # Cuando se han contado 5 parpadeos, se intenta reconocer al usuario mediante FaceNet
            if contador_pestaneos >= 5:
                rostro_recortado = crop_face_from_frame(frame, landmarks_suavizados)
                nombre, dist_min = reconocer_facenet(rostro_recortado)
                if nombre is not None:
                    cv2.putText(frame_final, f'Reconocido: {nombre} (dist={dist_min:.3f})', (30, 140),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
                    print(f"Usuario reconocido: {nombre} (distancia={dist_min:.3f})")
                else:
                    cv2.putText(frame_final, f'No reconocido (dist={dist_min:.3f})', (30, 140),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
                    print(f"Usuario NO reconocido (distancia={dist_min:.3f})")
                contador_pestaneos = 0
