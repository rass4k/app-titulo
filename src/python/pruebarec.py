...
# CONFIGURACIÓN
# --------------------------
BASE_DATOS = "usuarios_facenet.json"  # Archivo JSON con los embeddings registrados
SMOOTH_FACTOR = 0.9
OBJETIVO_SEGUNDOS = 3         # Se necesitan 3 segundos estables
...
def reconocer_facenet():
    ...
    with mp_face_mesh.FaceMesh(
        ...
        while cap.isOpened():
            ...
            # Conteo de tiempo estable para confirmar captura
            if estable_en_este_frame:
                ...
                if tiempo_restante > 0:
                    ...
                    mensaje = "✅ Captura para reconocimiento completada."
                    ...
                    # Comparar el embedding actual con cada usuario en la base de datos
                    match = None
                    min_distance = float('inf')
                    for usuario in usuarios:
                        # Se normaliza el embedding almacenado antes de compararlo  
                        stored_embedding_norm = normalize(usuario["embedding_facial"])
                        dist = np.linalg.norm(embedding_norm - stored_embedding_norm)
                        if dist < min_distance:
                            min_distance = dist
                            match = usuario

                    # Definir un umbral para reconocimiento (ajustar según pruebas)
                    THRESHOLD = 0.8  # Este valor puede necesitar ajustes según el rendimiento

                    if min_distance < THRESHOLD:
                        mensaje = f"Usuario reconocido: {match['nombre']} (ID: {match['id']})\nDistancia: {min_distance:.3f}"
                        print(mensaje)
                    else:
                        mensaje = f"Usuario no reconocido. Distancia mínima: {min_distance:.3f}"
                        print(mensaje)
                    
                    # Mostrar el mensaje en pantalla y esperar para visualizar el resultado
                    cv2.putText(frame, mensaje, (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
